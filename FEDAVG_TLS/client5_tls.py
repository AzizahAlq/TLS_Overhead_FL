#!/usr/bin/env python3
"""
fedavg_mnist_client_tls_pftlstyle.py

TLS gRPC (server-auth only), but SAME control-flow style as your PFTL client:

  pull_global -> train -> push_update -> wait_new_round (poll until server_round advances) -> next

Goals:
- Same model / weights structure / pickle protocol as aggregator
- Strict-ish barrier behavior on client side (1 update per server_round; WAITING -> poll)
- Logs aligned like PFTL: bytes_received after the poll that advances the round
- Writes: client_0_tls_comm_log.csv, client_0_tls_resources.csv, client_0_final_metrics.csv

NO args/env (edit constants below).
"""
...
COMM_LOG  = f"{CLIENT_ID}_tls_comm_log.csv"
RES_LOG   = f"{CLIENT_ID}_tls_resources.csv"
FINAL_LOG = f"{CLIENT_ID}_tls_final_metrics.csv"

CA_CERT = "certs/ca.crt"
TLS_SERVER_NAME_OVERRIDE = "localhost"  # must match cert CN/SAN
# ==================================================
...
def make_tls_channel(target: str):
    ...
    if TLS_SERVER_NAME_OVERRIDE:
        opts.append(("grpc.ssl_target_name_override", TLS_SERVER_NAME_OVERRIDE))
...
def push_update(stub, server_round, weights_list, num_samples):
    """Like push_shared(): returns (bytes_sent, t0_perf, ack_status, ack_current_round)."""
...
def main():
    ...
    channel = make_tls_channel(TARGET)
    stub = myproto_pb2_grpc.AggregatorStub(channel)

    completed = 0
    print(f"[TLS-PFTLSTYLE {CLIENT_ID}] Need {NUM_ROUNDS} rounds. target={TARGET}")
    ...
    try:
        ...
        while completed < NUM_ROUNDS:
            ...
            # 4) barrier wait (PFTL-style alignment)
            rtt_sec = 0.0
            if ack_status == "WAITING":
                ...
                # server already advanced (barrier met). Pull until it changes (or timeout)
                new_round, new_w, rtt_sec, bytes_in2 = wait_new_round(stub, server_round, t0_perf)
                bytes_in = bytes_in2
                if new_w is not None and new_round > server_round:
                    model.set_weights(new_w)
                completed += 1

            else:
                # REJECTED/ERROR -> don't count
                pass
            ...
            print(f"[TLS-PFTLSTYLE] completed={completed}/{NUM_ROUNDS} "
    ...
    print(f"[FINAL-TLS] test_acc={row['test_acc']} peak_rss_mb={row['peak_rss_mb']} avg_cpu_norm%={row['avg_cpu_norm_percent']}")
